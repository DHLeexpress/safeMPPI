"""Window-native safe-flow expansion (separate module, 2026-07-14).

This module deliberately leaves ``grid_expand_hardtail.py`` unchanged.  Its
aggregation unit is one coherent executed H=10 window:

* Full / -Curriculum: task-space AND progress AND SOCP.
* -SOCP: task-space AND positive geometric clearance AND progress.
* -Progress: task-space AND SOCP.

Goal reach, collision later in the rollout, and another invalid sibling window
are diagnostics only.  They never veto an otherwise accepted window.
...
_HERE = os.path.dirname(os.path.abspath(__file__))          # codex_challenging/reference/
_STAGE = os.path.dirname(_HERE)                             # codex_challenging/
_REV = os.path.dirname(_STAGE)                              # rev_expansion/
_WORK = os.path.dirname(_REV)                               # overnight_run_07_06/
sys.path[:0] = [_HERE, _STAGE, _REV, _WORK]                 # local verifier/model first; shared grid last
...
class CurConfig:
    ...
    min_modes_per_gamma: int = 0   # trajectory homotopy is diagnostic under window-native aggregation
    ...
    # Gathering breadth is independent of the early optimization schedule.
    # A value of 0.5 retains the historical half-sized early/cooldown gather;
    # scene-specific runs can use 1.0 while still keeping the early demo mix.
    early_rollout_scale: float = 0.5
    ...
    # anchors / misc
    demo_frac: float = 0.0
    # Absolute-iteration demo schedule.  Example ((0, .50), (6, .25))
    # means iterations 1--5 use 50% expert rows and iteration 6 onward 25%.
    # The expert tensor source can be supplied directly so a scene-specific
    # demonstration is never confused with the legacy dataset prefix.
    demo_frac_schedule: tuple = ()
    demo_override_path: str = ""
    alpha: float = 0.0              # small signed loss on rejected coherent windows (0 = positive-only)
    neg_batch_cap: int = 16         # separate negative draw; does not reduce the positive/demo batch
    neg_windows_per_rollout: int = 24
    success_replay_quota: int = 0   # reached, accepted windows reserved inside the unchanged batch cap
    success_replay_cap: int = 3000
    success_windows_per_rollout: int = 32
    symmetry_augment: bool = False  # exact x<->y pair; scene driver enables only for symmetric geometry
    equivariance_weight: float = 0.0
    demo_stage_balanced: bool = False
    demo_stage_weights: tuple = (0.25, 0.25, 0.25, 0.25)
    eval_temp_cfm_weight: float = 0.0  # auxiliary CFM from eval-temperature base noise
    # Finite-NFE sampler calibration.  CFM fits the continuous velocity field,
    # but the deployed policy is the eight-step Euler map from
    # ``eval_temp * N(0,I)``.  This auxiliary loss differentiates through that
    # exact map on a small, reflection-paired subset of the unchanged batch.
    # It is disabled by default and never changes gathering temperature.
    eval_endpoint_eta: float = 0.0
    eval_endpoint_quota: int = 0
    eval_endpoint_scope: str = "all"  # all | pre | post | goal (within 1 m)
    eval_endpoint_gammas: tuple = ()  # empty = all; otherwise balance endpoint rows across this subset
    eval_goal_eta: float = 0.0       # exact H-step terminal goal/velocity loss on endpoint-selected rows
    eval_goal_velocity_weight: float = 0.2
    eval_mode_noise_coupling: bool = False
    lwf_eta: float = 0.0
    lwf_x0_scale: float = 1.0       # teacher consistency noise scale; 0.5 matches rollout visualization
    demo_cap: int = 1200
    ...
    resume_reset_optimizer: bool = False  # intentional phase boundary: retain state, clear Adam moments
    ...
    aggregation_semantics: str = "window_native_v1"
    # Optional controlled -Curriculum comparison.  The full arm writes an
    # iteration->accepted-window budget; -Curriculum can consume it so only
    # the split changes, not the number of positive windows presented.
    accepted_window_budget: dict | None = None
    active_window_budget: int | None = None
...
def _float_schedule_at(schedule, absolute_iter, default):
    """Piecewise-constant nonnegative float schedule in absolute iterations."""
    eligible = [float(v) for start, v in schedule if absolute_iter >= int(start)]
    value = eligible[-1] if eligible else float(default)
    if not np.isfinite(value) or value < 0.0:
        raise ValueError(f"scheduled float must be finite and nonnegative, got {value}")
    return value
...
def _fresh_batch_plan(n_e, n_f, mix, cap):
    ...
    # A named -Curriculum arm is genuinely single-class: a zero frontier
    # fraction means "draw easy only", not "the frontier class is missing".
    # For a two-class recipe, preserve the strict no-substitution behavior.
    if f_frac <= 0.0:
        return min(int(n_e), int(cap)), 0
    if e_frac <= 0.0:
        return 0, min(int(n_f), int(cap))
    if n_e == 0 or n_f == 0:
        return 0, 0                    # never substitute one required class for the other
    cands = []
    if e_frac > 0:
...
class SuccessReplay:
    """Bounded replay of accepted windows from reached trajectories.

    Rows remain window-valid under the arm that gathered them.  Whole-trajectory
    reach is used only for the replay curriculum, never as an acceptance gate.
    Per-rollout subsampling prevents one long completion from dominating.
    """

    def __init__(self, cap, windows_per_rollout=32, center=(2.5, 2.5), radius=1.2):
        self.cap = int(cap)
        self.windows_per_rollout = int(windows_per_rollout)
        self.center = np.asarray(center, dtype=float)
        self.radius = float(radius)
        self.T = None
        self.mode = self.stage = self.rid = self.it = None

    def __len__(self):
        return 0 if self.T is None else int(self.T["U"].shape[0])

    def add(self, fresh, iteration):
        reached = np.asarray(fresh.get("rollout_reached", []), dtype=bool)
        if not reached.any():
            return 0
        rids = np.asarray(fresh.get("rid"), dtype=int)
        selected = []
        for rid in sorted(set(rids[reached].tolist())):
            pool = np.where(reached & (rids == rid))[0]
            count = min(len(pool), self.windows_per_rollout)
            choose = np.rint(np.linspace(0, len(pool) - 1, count)).astype(int)
            selected.extend(pool[choose].tolist())
        idx_np = np.asarray(selected, dtype=int)
        idx = torch.as_tensor(idx_np, dtype=torch.long)
        new = {key: fresh[key][idx].detach().cpu().clone()
               for key in ("grid", "low5", "hist", "U", "gamma")}
        new_mode = np.asarray(fresh["mode"], dtype=object)[idx_np].copy()
        # Preserve the difficult part of a successful detour instead of
        # letting its longer pre/post diagonal dominate a tiny replay quota.
        # This is replay stratification only: rows remain independently
        # accepted H=10 windows and stage is never an acceptance predicate.
        states = np.asarray([
            GX2.state_from_low5(row)[:2] for row in new["low5"].numpy()
        ], dtype=float)
        radial = np.linalg.norm(states - self.center[None, :], axis=1)
        along = states.sum(axis=1) - self.center.sum()
        new_stage = np.where(
            radial <= self.radius + 0.55, "boundary",
            np.where(along < 0.0, "pre", "post"),
        ).astype(object)
        new_rid = int(iteration) * 1000 + rids[idx_np]
        new_it = np.full(len(idx_np), int(iteration), dtype=int)
        if self.T is None:
            self.T = new
            self.mode, self.stage, self.rid, self.it = new_mode, new_stage, new_rid, new_it
        else:
            self.T = {key: torch.cat([self.T[key], new[key]]) for key in self.T}
            self.mode = np.concatenate([self.mode, new_mode])
            self.stage = np.concatenate([self.stage, new_stage])
            self.rid = np.concatenate([self.rid, new_rid])
            self.it = np.concatenate([self.it, new_it])
        if len(self) > self.cap:
            cut = len(self) - self.cap
            self.T = {key: value[cut:] for key, value in self.T.items()}
            self.mode, self.stage = self.mode[cut:], self.stage[cut:]
            self.rid, self.it = self.rid[cut:], self.it[cut:]
        return len(idx_np)
    ...
    def draw(self, count):
        ...
        strata = np.asarray([
            f"{mode}|{stage}" for mode, stage in zip(self.mode, self.stage)
        ], dtype=object)
        return _draw_gamma_rid_balanced(
            np.arange(len(self)), int(count), self.T["gamma"].numpy(), self.rid, strata)

    def composition(self):
        if len(self) == 0:
            return {"gamma": {}, "mode": {}, "stage": {}, "mode_stage": {}}

        def counts(values):
            keys, vals = np.unique(np.asarray(values).astype(str), return_counts=True)
            return {str(key): int(value) for key, value in zip(keys, vals)}

        return {
            "gamma": counts(np.round(self.T["gamma"].numpy(), 6)),
            "mode": counts(self.mode),
            "stage": counts(self.stage),
            "mode_stage": counts([f"{m}|{s}" for m, s in zip(self.mode, self.stage)]),
        }

    def state_dict(self):
        return dict(
            cap=self.cap, windows_per_rollout=self.windows_per_rollout,
            center=self.center.copy(), radius=self.radius,
            T=None if self.T is None else {key: value.detach().cpu().clone()
                                          for key, value in self.T.items()},
            mode=self.mode, stage=self.stage, rid=self.rid, it=self.it,
        )
    ...
    def from_state_dict(cls, state):
        if state is None:
            return None
        replay = cls(
            state["cap"], state["windows_per_rollout"],
            center=state.get("center", (2.5, 2.5)), radius=state.get("radius", 1.2),
        )
        replay.T = state.get("T")
        replay.mode, replay.rid, replay.it = state.get("mode"), state.get("rid"), state.get("it")
        replay.stage = state.get("stage")
        # Deterministically upgrade earlier complete checkpoints; no replay
        # windows or optimizer/RNG state are discarded.
        if replay.T is not None and replay.stage is None:
            states = np.asarray([
                GX2.state_from_low5(row)[:2] for row in replay.T["low5"].numpy()
            ], dtype=float)
            radial = np.linalg.norm(states - replay.center[None, :], axis=1)
            along = states.sum(axis=1) - replay.center.sum()
            replay.stage = np.where(
                radial <= replay.radius + 0.55, "boundary",
                np.where(along < 0.0, "pre", "post"),
            ).astype(object)
        return replay
...
def _capture_train_state(iteration, opt, qbuf, covered, pile, teacher, history,
    ...
    return dict(
        ...
        success_replay=(None if success_replay is None else success_replay.state_dict()),
...
def _apply_train_state(state, opt, teacher, gammas, restore_rng=True, expected_signature=None,
                       allow_drift_keys=(), allow_missing_teacher=False):
    ...
    if teacher is not None and state.get("teacher_state") is None and not allow_missing_teacher:
    ...
    success_replay = SuccessReplay.from_state_dict(state.get("success_replay"))
    if teacher is not None and state.get("teacher_state") is not None:
        teacher.load_state_dict(state["teacher_state"])
    restored = dict(
        qbuf=qbuf, covered=covered, pile=pile, success_replay=success_replay,
...
def update_flow_fresh(policy, opt, fresh, easy_idx, frontier_idx, mix, n_steps, cfg,
                      ...
                      trust_anchor=None, env=None, escape_replay=None, success_replay=None):
    ...
    success_quota = int(getattr(cfg, "success_replay_quota", 0))
    k_success = (min(success_quota, max(fresh_target - 2, 0))
                 if success_replay is not None and len(success_replay) else 0)
    fresh_target -= k_success
    ...
    B = ne + nf + k_hard + k_guard + k_success
    if B == 0:
        return None
    ...
    policy.train()
    losses, neg_losses, equiv_losses, eval_endpoint_losses, eval_goal_losses, eval_endpoint_used = (
        [], [], [], [], [], [])
    eval_mode_label_known = []
    fgr, egr, rid_ns, rid_doms, batch_gamma_counts, batch_mode_counts = (
        [], [], [], [], [], [])
    functional_steps, anchor_drifts, rollback_count = [], [], 0
    hard_used, hard_x0_used, hard_oob_fracs, escape_used, success_used = 0, 0, [], 0, 0
    for _ in range(n_steps):
        Gs, Ls, Hs, Us, rid_all, route_label_chunks, gamma_label_chunks = [], [], [], [], [], [], []
        parts = []
        ...
        if ne_fr > 0:
            parts.append(_draw_gamma_rid_balanced(easy_regular, ne_fr, fresh_gamma, rids, fresh_modes)
                         if (fresh_gamma is not None and rids is not None)
                         else np.random.choice(easy_regular, ne_fr, replace=True))
        if nf_fr > 0:
            parts.append(_draw_gamma_rid_balanced(frontier_regular, nf_fr, fresh_gamma, rids, fresh_modes)
                         if (fresh_gamma is not None and rids is not None)
        ...
        if parts:
            ...
            route_label_chunks.append(torch.as_tensor(
                ([_stable_route_label(fresh_modes[row]) for row in bi_np]
                 if fresh_modes is not None else [0.0] * len(bi_np)), dtype=torch.float32))
            gamma_label_chunks.append(torch.as_tensor(fresh_gamma[bi_np], dtype=torch.float32))
            if rids is not None:
        ...
        if ne_pl > 0 or nf_pl > 0:                          # pile part: LRU without-replacement draw
            ...
            if len(pi_np):
                ...
                route_label_chunks.append(torch.zeros(len(pi_np), dtype=torch.float32))
                gamma_label_chunks.append(pile.T["gamma"][pi].float())
                rid_all.append(pile.rid[pi_np])
        if k_success > 0:
            si_np = success_replay.draw(k_success)
            si = torch.as_tensor(si_np, dtype=torch.long)
            Gs.append(success_replay.T["grid"][si]); Ls.append(success_replay.T["low5"][si])
            Hs.append(success_replay.T["hist"][si]); Us.append(success_replay.T["U"][si])
            route_label_chunks.append(torch.as_tensor(
                [_stable_route_label(success_replay.mode[row]) for row in si_np], dtype=torch.float32))
            gamma_label_chunks.append(success_replay.T["gamma"][si].float())
            rid_all.append(success_replay.rid[si_np])
            success_used += len(si_np)
        G = torch.cat(Gs).to(device); L = torch.cat(Ls).to(device)
        H = torch.cat(Hs).to(device); U = torch.cat(Us).to(device)
        route_labels = torch.cat(route_label_chunks).to(device)
        ...
        if nd_demo > 0:                                        # δ anchor: mix pretraining-demo windows in
            di = _draw_demo_rows(
                demo, nd_demo, getattr(cfg, "demo_stage_weights", (0.25,) * 4))
            ...
            demo_modes = np.asarray(demo.get("sample_mode", ["unknown"] * nd), dtype=object)
            route_labels = torch.cat([
                route_labels,
                torch.as_tensor([_stable_route_label(demo_modes[row]) for row in di.numpy()],
                                dtype=torch.float32, device=device),
            ])
            gamma_label_chunks.append(demo["gamma"][di].float())
        gamma_labels = torch.cat(gamma_label_chunks).to(device)
        if len(route_labels) != len(U):
            raise RuntimeError("stable route-label rows do not match the assembled training batch")
        if len(gamma_labels) != len(U):
            raise RuntimeError("gamma-label rows do not match the assembled training batch")
        eval_mode_label_known.append(float((route_labels != 0).float().mean()))
        ...
        if bool(getattr(cfg, "symmetry_augment", False)):
            positive_loss, _, equivariance_loss = _symmetric_cfm_loss_x0(
                policy, G, L, H, U, getattr(cfg, "equivariance_weight", 0.0),
                x0_override=(x0_map or None),
            )
            eval_temp_weight = float(getattr(cfg, "eval_temp_cfm_weight", 0.0))
            if eval_temp_weight > 0.0:
                eval_loss, _, eval_equivariance = _symmetric_cfm_loss_x0(
                    policy, G, L, H, U, getattr(cfg, "equivariance_weight", 0.0),
                    x0_scale=float(cfg.eval_temp),
                    mode_noise_coupling=bool(getattr(cfg, "eval_mode_noise_coupling", False)),
                    mode_labels=route_labels,
                )
                positive_loss = (
                    positive_loss + eval_temp_weight * eval_loss
                ) / (1.0 + eval_temp_weight)
                equivariance_loss = (
                    equivariance_loss + eval_temp_weight * eval_equivariance
                ) / (1.0 + eval_temp_weight)
            equiv_losses.append(float(equivariance_loss))
        else:
            positive_loss = _cfm_loss_x0(
                policy, U, policy.ctx_from(G, L, H), x0_override=(x0_map or None))
        loss = float(getattr(cfg, "cfm_eta", 1.0)) * positive_loss
        endpoint_eta = float(getattr(cfg, "eval_endpoint_eta", 0.0))
        endpoint_quota = min(int(getattr(cfg, "eval_endpoint_quota", 0)), len(U))
        if endpoint_eta > 0.0 and endpoint_quota > 0:
            # The batch itself is already curriculum/replay/demo balanced.  A
            # fresh subset per inner step therefore exposes the endpoint map
            # to all gammas, stages, and route modes without increasing the
            # configured batch or replacing any CFM row.
            endpoint_pool = torch.arange(len(U), device=U.device)
            endpoint_scope = str(getattr(cfg, "eval_endpoint_scope", "all"))
            if endpoint_scope in ("pre", "post", "goal"):
                goal = torch.as_tensor(GM2.GOAL_XY, device=L.device, dtype=L.dtype)
                center = torch.as_tensor(getattr(cfg, "giant_center", (2.5, 2.5)),
                                         device=L.device, dtype=L.dtype)
                position = goal[None] - L[:, :2] * float(GF.R_GOAL)
                if endpoint_scope == "goal":
                    goal_distance = torch.linalg.vector_norm(position - goal[None], dim=1)
                    endpoint_pool = torch.where(goal_distance <= 1.0)[0]
                else:
                    radial = torch.linalg.vector_norm(position - center[None], dim=1)
                    outside_boundary = radial > float(getattr(cfg, "giant_radius", 1.2)) + 0.55
                    requested_stage = ((position.sum(dim=1) < center.sum()) if endpoint_scope == "pre"
                                       else (position.sum(dim=1) >= center.sum()))
                    endpoint_pool = torch.where(outside_boundary & requested_stage)[0]
            elif endpoint_scope != "all":
                raise ValueError(f"unknown eval endpoint scope: {endpoint_scope}")
            endpoint_gammas = tuple(float(g) for g in getattr(cfg, "eval_endpoint_gammas", ()))
            if endpoint_gammas and len(endpoint_pool):
                gamma_mask = torch.zeros(len(endpoint_pool), dtype=torch.bool, device=U.device)
                for gamma in endpoint_gammas:
                    gamma_mask |= torch.isclose(
                        gamma_labels[endpoint_pool],
                        torch.as_tensor(gamma, device=U.device, dtype=gamma_labels.dtype),
                    )
                endpoint_pool = endpoint_pool[gamma_mask]
            if len(endpoint_pool):
                if endpoint_gammas:
                    chunks = []
                    nonempty = [
                        endpoint_pool[torch.isclose(
                            gamma_labels[endpoint_pool],
                            torch.as_tensor(gamma, device=U.device, dtype=gamma_labels.dtype),
                        )]
                        for gamma in endpoint_gammas
                    ]
                    nonempty = [rows for rows in nonempty if len(rows)]
                    for j, rows in enumerate(nonempty):
                        count = endpoint_quota // len(nonempty) + (j < endpoint_quota % len(nonempty))
                        order = torch.randint(len(rows), (count,), device=U.device)
                        chunks.append(rows[order])
                    endpoint_rows = torch.cat(chunks) if chunks else endpoint_pool[:0]
                else:
                    endpoint_rows = endpoint_pool[
                        torch.randperm(len(endpoint_pool), device=U.device)[:min(endpoint_quota, len(endpoint_pool))]
                    ]
                goal_eta = float(getattr(cfg, "eval_goal_eta", 0.0))
                endpoint_result = _symmetric_eval_endpoint_loss(
                    policy,
                    G[endpoint_rows], L[endpoint_rows], H[endpoint_rows], U[endpoint_rows],
                    x0_scale=float(cfg.eval_temp), nfe=int(cfg.nfe_explore),
                    mode_noise_coupling=bool(getattr(cfg, "eval_mode_noise_coupling", False)),
                    mode_labels=route_labels[endpoint_rows],
                    return_goal_loss=goal_eta > 0.0,
                    dt=float(getattr(env, "dt", 0.1)),
                    goal_velocity_weight=float(getattr(cfg, "eval_goal_velocity_weight", 0.2)),
                )
                if goal_eta > 0.0:
                    endpoint_loss, goal_loss = endpoint_result
                    loss = loss + goal_eta * goal_loss
                    eval_goal_losses.append(float(goal_loss.detach()))
                else:
                    endpoint_loss = endpoint_result
                loss = loss + endpoint_eta * endpoint_loss
                eval_endpoint_losses.append(float(endpoint_loss.detach()))
                eval_endpoint_used.append(int(len(endpoint_rows)))
        alpha = float(getattr(cfg, "alpha", 0.0))
        negative = fresh.get("negative") if fresh is not None else None
        if alpha > 0.0 and negative is not None and negative["U"].shape[0] > 0:
            n_neg = min(int(getattr(cfg, "neg_batch_cap", cfg.batch_cap)), negative["U"].shape[0])
            ni = torch.randperm(negative["U"].shape[0])[:n_neg]
            Gn = negative["grid"][ni].to(device); Ln = negative["low5"][ni].to(device)
            Hn = negative["hist"][ni].to(device); Un = negative["U"][ni].to(device)
            negative_loss = _cfm_loss_x0(policy, Un, policy.ctx_from(Gn, Ln, Hn))
            loss = loss - alpha * negative_loss
            neg_losses.append(float(negative_loss.detach()))
        ...
        if cfg.lwf_eta > 0 and teacher is not None and demo is not None:   # η anchor: LwF on demo contexts
            ...
            x0 = float(getattr(cfg, "lwf_x0_scale", 1.0)) * torch.randn_like(x1)
    ...
    return dict(loss=float(np.mean(losses)) if losses else float("nan"),
                ...
                cfm_eta=float(getattr(cfg, "cfm_eta", 1.0)),
                alpha=float(getattr(cfg, "alpha", 0.0)),
                equivariance_loss=(float(np.mean(equiv_losses)) if equiv_losses else float("nan")),
                eval_endpoint_loss=(float(np.mean(eval_endpoint_losses))
                                    if eval_endpoint_losses else float("nan")),
                eval_endpoint_eta=float(getattr(cfg, "eval_endpoint_eta", 0.0)),
                eval_endpoint_quota=int(getattr(cfg, "eval_endpoint_quota", 0)),
                eval_endpoint_scope=str(getattr(cfg, "eval_endpoint_scope", "all")),
                eval_endpoint_gammas=list(getattr(cfg, "eval_endpoint_gammas", ())),
                eval_goal_loss=(float(np.mean(eval_goal_losses))
                                if eval_goal_losses else float("nan")),
                eval_goal_eta=float(getattr(cfg, "eval_goal_eta", 0.0)),
                eval_goal_velocity_weight=float(getattr(cfg, "eval_goal_velocity_weight", 0.2)),
                eval_endpoint_used=(float(np.mean(eval_endpoint_used)) if eval_endpoint_used else 0.0),
                eval_mode_noise_coupling=bool(getattr(cfg, "eval_mode_noise_coupling", False)),
                eval_mode_label_known=(float(np.mean(eval_mode_label_known))
                                       if eval_mode_label_known else 0.0),
                negative_loss=float(np.mean(neg_losses)) if neg_losses else float("nan"),
                batch_neg=(min(int(getattr(cfg, "neg_batch_cap", cfg.batch_cap)),
                               fresh.get("negative", {}).get("U", torch.empty(0)).shape[0])
                           if fresh is not None else 0),
                batch_success=int(k_success), success_replay_used=int(success_used),
                success_replay_size=(len(success_replay) if success_replay is not None else 0),
...
def _window_acceptance(low5, controls, env, gamma, cfg):
    """Evaluate exactly one coherent executed window.

    The returned predicate values distinguish an omitted ablation predicate
    (``None``) from a checked predicate that failed (``False``).  This is both
    the gather authority and the audit authority; no trajectory-level status
    is consulted here.
    """
    low5 = np.asarray(low5)
    controls = np.asarray(controls)
    progress, points, distances = _window_progress(low5, controls, env)
    taskspace_ok = bool(GM.in_taskspace(points))
    result = dict(
        accepted=False,
        progress=float(progress),
        taskspace_ok=taskspace_ok,
        progress_ok=None,
        progress_floor_ok=None,
        socp_ok=None,
        safe_space_ok=None,
        margin=float("nan"),
        residual=float("nan"),
        failure=(None if taskspace_ok else "taskspace"),
    )
    if not taskspace_ok:
        return result

    if not bool(getattr(cfg, "ablate_progress", False)):
        result["progress_ok"] = bool(GM2.approach_ok(distances))
        floor = min(float(cfg.valid_prog_floor), 0.5 * float(distances[0]))
        result["progress_floor_ok"] = bool(progress >= floor)
        if not result["progress_ok"]:
            result["failure"] = "progress"
            return result
        if not result["progress_floor_ok"]:
            result["failure"] = "progress_floor"
            return result

    state = GX2.state_from_low5(low5)
    if bool(getattr(cfg, "ablate_socp", False)):
        # -SOCP still means a physically safe local sample.  It omits the
        # multi-step certificate, not collision/free-space validity.
        margin = float(GM2.window_min_clearance(state, controls, env))
        safe = bool(np.isfinite(margin) and margin > 1e-6)
        result.update(safe_space_ok=safe, margin=margin, residual=0.0)
        if not safe:
            result["failure"] = "safe_space"
            return result
    else:
        plan_ok, margin, residual = GM2.window_socp_stats(state, controls, env, float(gamma))
        result.update(socp_ok=bool(plan_ok), safe_space_ok=bool(plan_ok),
                      margin=float(margin), residual=float(residual))
        if not plan_ok:
            result["failure"] = "socp"
            return result
...
def _balanced_window_prefix(gammas, budget):
    """Deterministic gamma-round-robin prefix used by controlled -Curriculum."""
    gammas = np.asarray(gammas, dtype=float)
    budget = int(budget)
    if budget < 0:
        raise ValueError("accepted-window budget must be nonnegative")
    if budget >= len(gammas):
        return np.arange(len(gammas), dtype=int)
    queues = [list(np.where(np.isclose(gammas, g))[0]) for g in np.unique(gammas)]
    out = []
    while len(out) < budget and any(queues):
        for queue in queues:
            if queue and len(out) < budget:
                out.append(queue.pop(0))
    return np.asarray(out, dtype=int)


def _slice_window_fields(fresh, indices):
    """Slice only per-window fields while retaining rollout/audit metadata."""
    indices = np.asarray(indices, dtype=int)
    window_fields = {
        "grid", "low5", "hist", "U", "gamma", "prog", "rid", "socp_margin",
        "cert_residual", "widx", "mode", "proposal_target", "rkind", "strip",
        "whole_traj_valid", "rollout_reached",
    }
    out = {}
    for key, value in fresh.items():
        out[key] = value[indices] if key in window_fields else value
    return out
...
def _reached_route_mode(path, cfg):
    """Scene-aware name for a reached trajectory's global mode."""
    path = np.asarray(path, dtype=float)
    if str(getattr(cfg, "scene_name", "")).startswith("giant_obstacle"):
        center = np.asarray(getattr(cfg, "giant_center", (2.5, 2.5)), dtype=float)
        radius = float(getattr(cfg, "giant_radius", 1.2))
        radial = np.linalg.norm(path - center[None, :], axis=1)
        near = radial <= radius + 0.45
        if not near.any():
            near[int(np.argmin(radial))] = True
        side_score = float(np.median(path[near, 1] - path[near, 0]))
        return "upper-left" if side_score >= 0.0 else "lower-right"
    return GM.staircase_id(path, reach=float(getattr(cfg, "reach", 0.1)))


def _gather_fresh(policy, unc, env, cfg, gammas, beta, K, target_e, target_f, qbuf, covered, device,
                  gamma_offset=0):
    """Gather coherent locally accepted windows from every queried rollout.

    Whole-rollout validity and goal reach are recorded only for diagnostics.  A
    later collision or a failing sibling window cannot discard a passing H=10
    window.  ``valid`` therefore means "rollouts contributing at least one
    accepted window", not whole-valid trajectories.
    """
    if qbuf is not None:
    ...
    gG, gL, gH, gU, prog, socp_margin, cert_residual, rid, widx, wgamma, modes, proposal_targets = (
        [], [], [], [], [], [], [], [], [], [], [], [])
    nG, nL, nH, nU, negative_failures = [], [], [], [], []
    window_whole_valid, window_reached = [], []
    ...
    audit = dict(aggregation_semantics="window_native_v1",
                 queried_rollouts=0, queried_windows=0, strict_reached=0,
                 whole_valid2_pass=0, whole_valid2_fail=0,
                 coherent_windows_total=0, coherent_windows_checked=0,
                 coherent_windows_certified=0, accepted_windows=0,
                 accepted_from_whole_invalid=0, accepted_from_unreached=0,
                 contributor_rollouts=0, contributor_whole_invalid=0,
                 contributor_unreached=0, window_failures=Counter(),
                 taskspace_pass=0, progress_evaluated=0, progress_pass=0,
                 socp_evaluated=0, socp_pass=0,
                 safe_space_evaluated=0, safe_space_pass=0,
    ...
    window_budget = getattr(cfg, "active_window_budget", None)
    if window_budget is not None:
        window_budget = int(window_budget)
        if window_budget < 0:
            raise ValueError("active accepted-window budget must be nonnegative")
        audit["requested_window_budget"] = window_budget
        if window_budget == 0:
            audit.update(ready=True, accepted_windows_pre_cap=0, accepted_windows=0,
                         valid_rollouts=0, window_failures={})
            return None, qbuf, reached, coll, 0, 0, audit
    ...
    while att < max_att:
        amount_ready = ((len(wgamma) >= window_budget) if window_budget is not None else (valid >= K_eff))
        if (amount_ready and classes_ready and (emergent or (gamma_ready and gamma_class_ready)) and
        ...
        audit["whole_valid2_pass" if traj_ok else "whole_valid2_fail"] += 1
        pth = np.asarray(out["path"], dtype=float)
        # No trajectory-level dither/progress/reach gate is legal in this
        # module.  The enabled predicates below are evaluated per window.
        coherent = _executed_horizon_tensors(out["recs"])
        if coherent is None:
            continue
        G, L, H, U = coherent
        keep, wp, wm, wr, rejected, rejected_failures = [], [], [], [], [], []
        for i in range(U.shape[0]):
            audit["coherent_windows_total"] += 1
            decision = _window_acceptance(L[i].numpy(), U[i].numpy(), env, float(g), cfg)
            audit["taskspace_pass"] += int(decision["taskspace_ok"])
            if decision["progress_ok"] is not None:
                audit["progress_evaluated"] += 1
                audit["progress_pass"] += int(decision["progress_ok"] and
                                                decision["progress_floor_ok"])
            if decision["socp_ok"] is not None:
                audit["socp_evaluated"] += 1
                audit["socp_pass"] += int(decision["socp_ok"])
            if decision["safe_space_ok"] is not None:
                audit["safe_space_evaluated"] += 1
                audit["safe_space_pass"] += int(decision["safe_space_ok"])
            if not decision["accepted"]:
                audit["window_failures"][decision["failure"]] += 1
                rejected.append(i)
                rejected_failures.append(str(decision["failure"]))
                continue
            audit["coherent_windows_checked"] += 1
            audit["coherent_windows_certified"] += 1
            keep.append(i)
            wp.append(decision["progress"])
            wm.append(decision["margin"])
            wr.append(decision["residual"])
        if rejected:
            cap = max(1, int(getattr(cfg, "neg_windows_per_rollout", 24)))
            if len(rejected) > cap:
                choose = np.rint(np.linspace(0, len(rejected) - 1, cap)).astype(int)
                rejected = [rejected[index] for index in choose]
                rejected_failures = [rejected_failures[index] for index in choose]
            ni = torch.as_tensor(rejected, dtype=torch.long)
            nG.append(G[ni]); nL.append(L[ni]); nH.append(H[ni]); nU.append(U[ni])
            negative_failures.extend(rejected_failures)
        if not keep:
            continue
        audit["accepted_windows"] += len(keep)
        audit["accepted_from_whole_invalid"] += (0 if traj_ok else len(keep))
        audit["accepted_from_unreached"] += (0 if out["reached"] else len(keep))
        audit["contributor_rollouts"] += 1
        audit["contributor_whole_invalid"] += int(not traj_ok)
        audit["contributor_unreached"] += int(not out["reached"])
        ...
        sid = (_reached_route_mode(pth, cfg)
        ...
        window_whole_valid.extend([bool(traj_ok)] * len(keep))
        window_reached.extend([bool(out["reached"])] * len(keep))
        ...
        partial = dict(grid=torch.cat(gG), low5=torch.cat(gL), hist=torch.cat(gH), U=torch.cat(gU),
                       ...
                       whole_traj_valid=np.asarray(window_whole_valid, dtype=bool),
                       rollout_reached=np.asarray(window_reached, dtype=bool),
    ...
    if nG:
        fresh["negative"] = dict(
            grid=torch.cat(nG), low5=torch.cat(nL), hist=torch.cat(nH), U=torch.cat(nU),
            failure=np.asarray(negative_failures, dtype=object),
        )
    amount_ready = ((fresh["U"].shape[0] >= window_budget)
                    if window_budget is not None else (valid >= K_eff))
    ready = (amount_ready and classes_ready and (emergent or (gamma_ready and gamma_class_ready)) and
             mode_ready and target_hit_ready)
    audit["accepted_windows_pre_cap"] = int(fresh["U"].shape[0])
    audit["negative_windows"] = int(
        fresh.get("negative", {}).get("U", torch.empty(0)).shape[0]
    )
    audit["negative_failure_counts"] = dict(Counter(negative_failures))
    if ready and window_budget is not None and fresh["U"].shape[0] > window_budget:
        sel = _balanced_window_prefix(fresh["gamma"].numpy(), window_budget)
        fresh = _slice_window_fields(fresh, sel)
    if ready:
        audit["accepted_windows"] = int(fresh["U"].shape[0])
        audit["accepted_from_whole_invalid"] = int((~fresh["whole_traj_valid"]).sum())
        audit["accepted_from_unreached"] = int((~fresh["rollout_reached"]).sum())
    ...
    audit["window_failures"] = dict(audit["window_failures"])
...
def _load_demo(cfg):
    override = str(getattr(cfg, "demo_override_path", "") or "")
    if override:
        payload = torch.load(override, map_location="cpu", weights_only=False)
        missing = [key for key in ("grid", "low5", "hist", "U") if key not in payload]
        if missing:
            raise KeyError(f"demo override {override} misses {missing}")
        demo = {key: torch.as_tensor(payload[key]).float().cpu()
                for key in ("grid", "low5", "hist", "U")}
        for key in ("gamma", "trajectory_id", "window_step", "source_seed"):
            if key in payload:
                demo[key] = torch.as_tensor(payload[key]).cpu()
        n = demo["U"].shape[0]
        if any(value.shape[0] != n for value in demo.values() if torch.is_tensor(value)):
            raise ValueError(f"demo override {override} has inconsistent row counts")
        if tuple(demo["U"].shape[1:]) != (GF.H_PRED, 2):
            raise ValueError(f"demo override U shape {tuple(demo['U'].shape)} is not [N,{GF.H_PRED},2]")
        if bool(getattr(cfg, "demo_stage_balanced", False)):
            required = ("gamma", "trajectory_id", "window_step")
            missing_meta = [key for key in required if key not in demo]
            if missing_meta:
                raise KeyError(f"stage-balanced demo sampling requires {missing_meta}")
            positions = np.asarray([
                GX2.state_from_low5(row)[:2] for row in demo["low5"].numpy()
            ], dtype=float)
            center = np.asarray(getattr(cfg, "giant_center", (2.5, 2.5)), dtype=float)
            radius = float(getattr(cfg, "giant_radius", 1.2))
            radial = np.linalg.norm(positions - center[None, :], axis=1)
            along = positions.sum(axis=1) - center.sum()
            steps = demo["window_step"].numpy()
            stage = np.where(
                steps == 0, "initial",
                np.where(radial <= radius + 0.55, "boundary",
                         np.where(along < 0.0, "approach", "post")),
            ).astype(object)
            trajectory = demo["trajectory_id"].numpy().astype(int)
            mode_by_trajectory = {}
            for rid in np.unique(trajectory):
                rows = np.where(trajectory == rid)[0]
                near = rows[radial[rows] <= radius + 0.55]
                if not len(near):
                    near = rows[np.argsort(radial[rows])[:max(1, min(8, len(rows)))]]
                side = float(np.median(positions[near, 1] - positions[near, 0]))
                mode_by_trajectory[int(rid)] = "upper-left" if side >= 0.0 else "lower-right"
            demo["sample_stage"] = stage
            demo["sample_mode"] = np.asarray(
                [mode_by_trajectory[int(rid)] for rid in trajectory], dtype=object)
        return demo
...
def _draw_demo_rows(demo, count, stage_weights=(0.25, 0.25, 0.25, 0.25)):
    """Draw gamma/mode/trajectory-balanced rows with equal route-stage mass."""
    if count <= 0:
        return torch.empty(0, dtype=torch.long)
    if "sample_stage" not in demo:
        return torch.randint(0, demo["U"].shape[0], (count,))
    stages = ("initial", "approach", "boundary", "post")
    weights = np.asarray(stage_weights, dtype=float)
    if weights.shape != (len(stages),) or not np.isfinite(weights).all() or (weights < 0).any():
        raise ValueError(f"demo_stage_weights must be four finite nonnegative values, got {stage_weights}")
    if weights.sum() <= 0:
        raise ValueError("demo_stage_weights must have positive mass")
    raw = int(count) * weights / weights.sum()
    quotas = np.floor(raw).astype(int)
    remainder = int(count) - int(quotas.sum())
    # Random tie-breaking prevents the two equal low-mass stages from always
    # assigning the odd slot to the same side of the trajectory.
    ties = np.random.random(len(stages)) * 1e-9
    for index in np.argsort(-(raw - quotas + ties))[:remainder]:
        quotas[index] += 1
    selected = []
    gamma = demo["gamma"].numpy()
    trajectory = demo["trajectory_id"].numpy().astype(int)
    mode = np.asarray(demo["sample_mode"], dtype=object)
    labels = np.asarray(demo["sample_stage"], dtype=object)
    for stage, quota in zip(stages, quotas):
        if not quota:
            continue
        pool = np.where(labels == stage)[0]
        if not len(pool):
            raise RuntimeError(f"stage-balanced demo pool has no {stage} rows")
        selected.extend(_draw_gamma_rid_balanced(
            pool, quota, gamma, trajectory, mode,
        ).tolist())
...
def _reflect_xy_batch(grid, low5, hist, controls):
    """Exact x<->y reflection for the axis-aligned polar context."""
    ...
    swap = torch.tensor((1, 0), device=hist.device)
    return (reflected_grid, reflected_low, hist.index_select(-1, swap),
            controls.index_select(-1, swap))


def _reflect_flat_controls(values):
    return values.reshape(len(values), GF.H_PRED, 2).index_select(
        -1, torch.tensor((1, 0), device=values.device)
    ).reshape(len(values), -1)


def _stable_route_label(mode):
    text = str(mode)
    if text == "lower-right":
        return 1.0
    if text == "upper-left":
...
def _couple_x0_to_control_mode(x0, controls, mode_labels=None):
    """Couple only the reflection-antisymmetric source bit to the target route.

    Swapping the first latent action's x/y components preserves an iid
    Gaussian source.  The exact reflected training partner supplies the
    opposite route with the opposite source bit, so aggregate source mass
    remains balanced rather than collapsing onto either detour.
    """
    displacement_bit = controls[..., 0].sum(dim=1) - controls[..., 1].sum(dim=1)
    first_bit = controls[:, 0, 0] - controls[:, 0, 1]
    fallback = torch.where(displacement_bit.abs() > 1e-6, displacement_bit, first_bit).sign()
    fallback = torch.where(fallback == 0, torch.ones_like(fallback), fallback)
    if mode_labels is None:
        route = fallback
    else:
        route = mode_labels.to(x0).sign()
        route = torch.where(route == 0, fallback, route)
    wrong = (x0[:, 0] - x0[:, 1]) * route.to(x0) < 0
    pair = x0[wrong, :2].clone()
    x0[wrong, 0] = pair[:, 1]
    x0[wrong, 1] = pair[:, 0]
    return x0


def _symmetric_cfm_loss_x0(policy, grid, low5, hist, controls, equivariance_weight,
                           x0_override=None, x0_scale=1.0, mode_noise_coupling=False,
                           mode_labels=None):
    """Pretraining-faithful paired reflection loss with paired Gaussian noise."""
    rgrid, rlow, rhist, rcontrols = _reflect_xy_batch(grid, low5, hist, controls)
    batch = len(controls)
    x1 = (controls / policy.u_max).reshape(batch, policy.d)
    reflected_x1 = (rcontrols / policy.u_max).reshape(batch, policy.d)
    x0 = float(x0_scale) * torch.randn_like(x1)
    if x0_override:
        for row, value in x0_override.items():
            if row < batch:
                x0[row] = value
    if mode_noise_coupling:
        x0 = _couple_x0_to_control_mode(x0, controls, mode_labels)
    reflected_x0 = _reflect_flat_controls(x0)
    ...
    prediction = policy.forward(x_tau, tau, policy._expand_ctx(context, batch))
    reflected_prediction = policy.forward(
        reflected_x_tau, tau, policy._expand_ctx(reflected_context, batch))
    target = x1 - x0
    reflected_target = reflected_x1 - reflected_x0
    cfm = 0.5 * (((prediction - target) ** 2).mean() +
                 ((reflected_prediction - reflected_target) ** 2).mean())
    equivariance = ((reflected_prediction - _reflect_flat_controls(prediction)) ** 2).mean()
    return cfm + float(equivariance_weight) * equivariance, cfm.detach(), equivariance.detach()


def _symmetric_eval_endpoint_loss(policy, grid, low5, hist, controls, *, x0_scale, nfe,
                                  mode_noise_coupling=False, mode_labels=None,
                                  return_goal_loss=False, dt=0.1,
                                  goal_velocity_weight=0.2):
    """Loss on the exact finite-NFE sampler used by temperature-0.5 rollout evaluation.

    Every latent/target pair is accompanied by its exact x/y reflection.  This
    prevents endpoint calibration from reintroducing the upward bias that the
    balanced Stage-3 objective removed.  The target rows are accepted windows;
    no rejected trajectory or weakened validity predicate enters this loss.
    """
    if int(nfe) <= 0:
        raise ValueError("endpoint calibration requires a positive NFE")
    rgrid, rlow, rhist, rcontrols = _reflect_xy_batch(grid, low5, hist, controls)
    batch = len(controls)
    x = float(x0_scale) * torch.randn(batch, policy.d, device=controls.device,
                                      dtype=controls.dtype)
    if mode_noise_coupling:
        x = _couple_x0_to_control_mode(x, controls, mode_labels)
    reflected_x = _reflect_flat_controls(x)
    context = policy.ctx_from(grid, low5, hist)
    reflected_context = policy.ctx_from(rgrid, rlow, rhist)
    step = 1.0 / int(nfe)
    for index in range(int(nfe)):
        tau = torch.full((batch,), index / int(nfe), device=x.device, dtype=x.dtype)
        x = x + step * policy.forward(x, tau, policy._expand_ctx(context, batch))
        reflected_x = reflected_x + step * policy.forward(
            reflected_x, tau, policy._expand_ctx(reflected_context, batch))
    target = (controls / policy.u_max).reshape(batch, policy.d)
    reflected_target = (rcontrols / policy.u_max).reshape(batch, policy.d)
    endpoint_loss = 0.5 * (((x - target) ** 2).mean() +
                           ((reflected_x - reflected_target) ** 2).mean())
    if not return_goal_loss:
        return endpoint_loss

    def terminal_loss(sampled, conditioned_low5):
        sampled_controls = sampled.reshape(batch, GF.H_PRED, 2) * float(policy.u_max)
        goal = torch.as_tensor(GM2.GOAL_XY, device=sampled.device, dtype=sampled.dtype)
        position = goal[None] - conditioned_low5[:, :2] * float(GF.R_GOAL)
        velocity = conditioned_low5[:, 2:4] * float(GF.V_SCALE)
        for action in sampled_controls.unbind(dim=1):
            position = position + float(dt) * velocity + 0.5 * float(dt) ** 2 * action
            velocity = velocity + float(dt) * action
        position_error = ((position - goal[None]) ** 2).sum(dim=1)
        velocity_error = (velocity ** 2).sum(dim=1)
        return (position_error + float(goal_velocity_weight) * velocity_error).mean()

    goal_loss = 0.5 * (terminal_loss(x, low5) + terminal_loss(reflected_x, rlow))
    return endpoint_loss, goal_loss
...
def _escape_probe(policy, env, cfg, device, M=8, T=60, g=0.5):
    ...
    with _preserve_torch_rng():
        for _ in range(M):
            out = GR.fm_deploy(policy, env, float(g), T=T, temp=cfg.eval_temp,
                               nfe=cfg.nfe_explore, device=device)
...
def _cov_probe(policy, env, cfg, device, M=50, g=0.5):
    ...
    with _preserve_torch_rng():
        rows, _, paths = SR.eval_policy(policy, env, gammas=[g], M=M, T_max=cfg.T, reach=cfg.reach,
                                        temp=cfg.eval_temp, device=device, keep_paths=M,
                                        log=lambda *a, **k: None)
    goal = env.goal.detach().cpu().numpy()
    ids = set()
    for p in paths[g]:
...
def _measure(policy, env, cfg, device):
    with _preserve_torch_rng():
        rows, agg, _ = SR.eval_policy(policy, env, gammas=list(cfg.gammas), M=cfg.M_measure, T_max=cfg.T,
                                      reach=cfg.reach, temp=cfg.eval_temp, device=device,
                                      log=lambda *a, **k: None)
    return rows, agg
...
def _save_viz_db(fresh, scores, easy_idx, frontier_idx, mix, path, it, cap=4096):
    ...
    db = dict(iter=it, mix=list(mix), label=list(label[sel]),
              ...
              whole_traj_valid=fresh.get("whole_traj_valid", np.zeros(n, bool))[sel],
              rollout_reached=fresh.get("rollout_reached", np.zeros(n, bool))[sel],
...
def _resume_signature(cfg, freeze_enc, enc_lr_mult):
    ...
    sig = dict(version=2, target="executed_closed_loop_horizon", margin="real_face_m",
                aggregation_semantics="window_native_v1",
                ...
                demo_frac_schedule=[list(x) for x in getattr(cfg, "demo_frac_schedule", ())],
                demo_override_path=os.path.abspath(str(getattr(cfg, "demo_override_path", "") or "")),
                cfm_eta=float(getattr(cfg, "cfm_eta", 1.0)),
                resume_reset_optimizer=bool(getattr(cfg, "resume_reset_optimizer", False)),
                alpha=float(getattr(cfg, "alpha", 0.0)),
                neg_batch_cap=int(getattr(cfg, "neg_batch_cap", 0)),
                neg_windows_per_rollout=int(getattr(cfg, "neg_windows_per_rollout", 0)),
                success_replay_quota=int(getattr(cfg, "success_replay_quota", 0)),
                success_replay_cap=int(getattr(cfg, "success_replay_cap", 0)),
                success_windows_per_rollout=int(getattr(cfg, "success_windows_per_rollout", 0)),
                lwf_eta=float(cfg.lwf_eta),
                lwf_x0_scale=float(getattr(cfg, "lwf_x0_scale", 1.0)),
                temp=float(cfg.temp), eval_temp=float(cfg.eval_temp),
                ...
                early_rollout_scale=float(getattr(cfg, "early_rollout_scale", 0.5)),
                ...
                accepted_window_budget_controlled=cfg.accepted_window_budget is not None)
    ...
    if getattr(cfg, "symmetry_augment", False):
        sig.update(symmetry_augment=True,
                   equivariance_weight=float(getattr(cfg, "equivariance_weight", 0.0)))
    if getattr(cfg, "demo_stage_balanced", False):
        sig.update(
            demo_stage_balanced=True,
            demo_stage_weights=list(getattr(cfg, "demo_stage_weights", (0.25,) * 4)),
        )
    if float(getattr(cfg, "eval_temp_cfm_weight", 0.0)) > 0.0:
        sig.update(eval_temp_cfm_weight=float(cfg.eval_temp_cfm_weight))
    if bool(getattr(cfg, "eval_mode_noise_coupling", False)):
        sig.update(eval_mode_noise_coupling=True)
    if float(getattr(cfg, "eval_endpoint_eta", 0.0)) > 0.0:
        sig.update(
...
def run_expand_cur(policy, env, cfg: CurConfig, device="cpu", outdir=None, log=print,
    ...
    recipe = dict(algorithm="and_quantile_fixed_absolute_schedule", start_iter=cfg.start_iter,
                  ...
                  early_rollout_scale=float(getattr(cfg, "early_rollout_scale", 0.5)),
                  batch=cfg.batch_cap, demo_frac=cfg.demo_frac,
                  demo_frac_schedule=[list(x) for x in getattr(cfg, "demo_frac_schedule", ())],
                  demo_override_path=os.path.abspath(str(getattr(cfg, "demo_override_path", "") or "")),
                  negative_unlearning=dict(
                      alpha=float(getattr(cfg, "alpha", 0.0)),
                      batch_cap=int(getattr(cfg, "neg_batch_cap", 0)),
                      windows_per_rollout=int(getattr(cfg, "neg_windows_per_rollout", 0)),
                      semantics="coherent_H10_windows_rejected_by_enabled_arm_predicates",
                  ),
                  success_replay=dict(
                      quota=int(getattr(cfg, "success_replay_quota", 0)),
                      cap=int(getattr(cfg, "success_replay_cap", 0)),
                      windows_per_rollout=int(getattr(cfg, "success_windows_per_rollout", 0)),
                      semantics="accepted_window_native_rows_from_reached_rollouts_only",
                      sampling="gamma_then_route_mode_stage_then_source_rollout_balanced",
                  ),
                  symmetry=dict(
                      enabled=bool(getattr(cfg, "symmetry_augment", False)),
                      transform="exact_x_y_reflection_with_paired_gaussian_noise",
                      equivariance_weight=float(getattr(cfg, "equivariance_weight", 0.0)),
                  ),
                  demo_sampling=("equal_initial_approach_boundary_post_then_gamma_mode_rid"
                                 if getattr(cfg, "demo_stage_balanced", False) else "uniform_rows"),
                  demo_stage_weights=list(getattr(cfg, "demo_stage_weights", (0.25,) * 4)),
                  cfm_eta=float(getattr(cfg, "cfm_eta", 1.0)),
                  resume_reset_optimizer=bool(getattr(cfg, "resume_reset_optimizer", False)),
                  eval_temp_cfm=dict(
                      weight=float(getattr(cfg, "eval_temp_cfm_weight", 0.0)),
                      base_noise_scale=float(cfg.eval_temp),
                      standard_base_noise_scale=1.0,
                      combination="normalized_weighted_average",
                      mode_noise_coupling=bool(getattr(cfg, "eval_mode_noise_coupling", False)),
                  ),
                  eval_endpoint=dict(
                      weight=float(getattr(cfg, "eval_endpoint_eta", 0.0)),
                      quota=int(getattr(cfg, "eval_endpoint_quota", 0)),
                      scope=str(getattr(cfg, "eval_endpoint_scope", "all")),
                      goal_scope_radius_m=1.0,
                      goal_weight=float(getattr(cfg, "eval_goal_eta", 0.0)),
                      goal_velocity_weight=float(getattr(cfg, "eval_goal_velocity_weight", 0.2)),
                      mode_noise_coupling=bool(getattr(cfg, "eval_mode_noise_coupling", False)),
                      base_noise_scale=float(cfg.eval_temp),
                      nfe=int(cfg.nfe_explore),
                      symmetry="exact_x_y_reflection_pair",
                      targets="accepted_rows_from_unchanged_batch",
                  ),
                  valid_prog_floor=cfg.valid_prog_floor,
                  aggregation_semantics="window_native_v1",
                  trajectory_status="diagnostic_only", goal_reach_gate=False,
                  enabled_window_predicates=dict(
                      taskspace=True,
                      progress=not bool(getattr(cfg, "ablate_progress", False)),
                      socp=not bool(getattr(cfg, "ablate_socp", False)),
                      geometric_clearance=bool(getattr(cfg, "ablate_socp", False))),
                  demo_backfill=False, lr=cfg.lr, lwf_eta=cfg.lwf_eta,
                  lwf_x0_scale=float(getattr(cfg, "lwf_x0_scale", 1.0)),
                  ...
                  strict_gather_reach=None, executed_gate="coherent_H10_enabled_window_predicates",
                  ...
                  coherent_coverage_proposals=dict(fraction=cfg.targeted_frac, n_target=cfg.n_target,
                                                   ...
                                                   acceptance="coherent_H10_enabled_window_predicates"),
                  ...
                  gather_temperature=cfg.temp, evaluation_temperature=cfg.eval_temp,
                  ...
                  hard_tail=dict(recovery_frac=cfg.recovery_frac,
                                 ...
                                 acceptance="coherent_H10_enabled_window_predicates_started_on_strip",
    ...
    log(f"[fixed_and_expand{('/'+tag) if tag else ''}] abs={cfg.start_iter}+{cfg.iters} "
        ...
        f"early_rollout_scale={getattr(cfg, 'early_rollout_scale', 0.5):g} "
        ...
        f"β={cfg.beta} temp={cfg.temp} eval_temp={cfg.eval_temp} "
        f"demo_frac={cfg.demo_frac} demo_schedule={cfg.demo_frac_schedule} "
        f"alpha={getattr(cfg, 'alpha', 0.0)} neg_batch={getattr(cfg, 'neg_batch_cap', 0)} "
        f"success_replay={getattr(cfg, 'success_replay_quota', 0)}/{getattr(cfg, 'success_replay_cap', 0)} "
        f"lwf_eta={cfg.lwf_eta} NO_DEMO_BACKFILL"
    ...
    success_replay = (SuccessReplay(
        getattr(cfg, "success_replay_cap", 3000),
        getattr(cfg, "success_windows_per_rollout", 32),
        center=getattr(cfg, "giant_center", (2.5, 2.5)),
        radius=getattr(cfg, "giant_radius", 1.2),
    ) if int(getattr(cfg, "success_replay_quota", 0)) > 0 else None)
    ...
    budget_path = os.path.join(outdir, "accepted_window_budget.json") if outdir else None
    accepted_window_budget_log = {}
    if budget_path and cfg.start_iter > 0 and os.path.exists(budget_path):
        with open(budget_path) as f:
            accepted_window_budget_log = {str(k): int(v) for k, v in json.load(f).items()}
    ...
    if resume_state is not None:
        ...
        _drift_keys = (("beta", "alpha", "mix_start", "mix_end", "quantile_schedule", "inner_steps", "lr",
                        "enc_lr_mult", "early_rollout_scale",
                        "demo_frac", "demo_frac_schedule",
                        "symmetry_augment", "equivariance_weight", "demo_stage_balanced",
                        "demo_stage_weights", "cfm_eta", "resume_reset_optimizer",
                        "lwf_eta", "lwf_x0_scale",
                        "eval_temp_cfm_weight", "eval_endpoint_eta", "eval_endpoint_quota",
                        "eval_endpoint_scope", "eval_endpoint_gammas", "eval_goal_eta",
                        "eval_goal_velocity_weight", "eval_mode_noise_coupling",
                        "ablate_socp", "ablate_progress", "ablate_curriculum",
                        "accepted_window_budget_controlled",
                        "phased_curriculum", "phase_sr_threshold", "phase_sr_patience",
                        "gp_buf", "qbuf_cap", "success_replay_quota")
                       if getattr(cfg, "resume_allow_recipe_drift", False) else ())
        rs = _apply_train_state(
            resume_state, opt, teacher, gammas, restore_rng=True,
            expected_signature=resume_sig, allow_drift_keys=_drift_keys,
            allow_missing_teacher=bool(
                getattr(cfg, "resume_allow_recipe_drift", False)
                and getattr(cfg, "resume_reset_optimizer", False)
            ),
        )
        qbuf, covered, pile = rs["qbuf"], rs["covered"], rs["pile"]
        success_replay = rs.get("success_replay")
        ...
        cooled = rs["cooled"]
        # Adam state restoration also restores its old param-group learning
        # rates.  An explicitly permitted LR phase change must therefore be
        # applied after load_state_dict while preserving all moments.
        if getattr(cfg, "resume_allow_recipe_drift", False):
            opt.param_groups[0]["lr"] = float(cfg.lr)
            if len(opt.param_groups) > 1:
                opt.param_groups[1]["lr"] = float(cfg.lr) * float(enc_lr_mult)
        reset_optimizer_now = (
            bool(getattr(cfg, "resume_reset_optimizer", False))
            and not bool((resume_state.get("resume_signature") or {}).get(
                "resume_reset_optimizer", False))
        )
        if reset_optimizer_now:
            opt.state.clear()
            log("[resume] cleared Adam moments at intentional objective phase boundary", flush=True)
    ...
    log(f"it{cfg.start_iter:05d} SR {agg0['SR']:.2f} CR {agg0['CR']:.2f} | resume baseline "
        f"(pretrained repr{getattr(policy, 'repr_dim', '?')}, temp={cfg.eval_temp:g})", flush=True)
    ...
    def train_state_at(iteration):
        return _capture_train_state(iteration, opt, qbuf, covered, pile, teacher, history,
                                    ...
                                    resume_signature=resume_sig, success_replay=success_replay)
    ...
    for local_t in range(1, cfg.iters + 1):
        ...
        final_iter = t
        beta = cfg.beta
        cfg.demo_frac = _float_schedule_at(
            getattr(cfg, "demo_frac_schedule", ()), t, cfg.demo_frac)
        ...
        if cfg.accepted_window_budget is not None:
            if t not in cfg.accepted_window_budget:
                raise KeyError(f"accepted-window budget has no entry for iteration {t}")
            cfg.active_window_budget = int(cfg.accepted_window_budget[t])
        else:
            cfg.active_window_budget = None
        K = cfg.rollouts_per_iter
        if t <= cfg.early_until or t >= cfg.cooldown_from:
            scale = float(getattr(cfg, "early_rollout_scale", 0.5))
            if not (0.0 < scale <= 1.0):
                raise ValueError(f"early_rollout_scale must be in (0,1], got {scale}")
            K_eff = max(1, int(np.ceil(K * scale)))
        else:
            K_eff = K
        ...
        if budget_path:
            accepted_window_budget_log[str(t)] = int(n_valid)
            tmp = budget_path + ".tmp"
            with open(tmp, "w") as f:
                json.dump(accepted_window_budget_log, f, indent=2, sort_keys=True)
        ...
        if prime_only or t <= cfg.warmup_gather:           # WARM-UP: query/GP memory only, NO gradient step
            ...
            upd = update_flow_fresh(policy, opt, fresh, easy_idx, frontier_idx, mix, inner, cfg,
                                    ...
                                    escape_replay=escape_replay, success_replay=success_replay)
            if upd is not None:
                last = upd
            ...
            if upd is not None:
                last = upd
                it_batch, it_pile = upd["batch"], upd.get("n_pile", 0)
        # Reached status is a replay-curriculum attribute only.  Add after the
        # update so current rows cannot be counted twice in the same iteration.
        success_added = 0
        if fresh is not None and success_replay is not None:
            success_added = success_replay.add(fresh, t)
            last["success_replay_size"] = len(success_replay)
            last["success_replay_added"] = int(success_added)
        ...
        if cfg.log_comp_every and t % cfg.log_comp_every == 0:   # micro diagnostics (user 2026-07-09 pattern)
            ...
            rec = dict(iter=t, beta=beta, quantile=cfg.active_quantile,
                       ...
                       cfm_eta=float(last.get("cfm_eta", getattr(cfg, "cfm_eta", 1.0))),
                       alpha=float(last.get("alpha", getattr(cfg, "alpha", 0.0))),
                       equivariance_loss=last.get("equivariance_loss", float("nan")),
                       eval_endpoint_loss=last.get("eval_endpoint_loss", float("nan")),
                       eval_endpoint_eta=last.get("eval_endpoint_eta", 0.0),
                       eval_endpoint_quota=last.get("eval_endpoint_quota", 0),
                       eval_endpoint_scope=last.get("eval_endpoint_scope", "all"),
                       eval_endpoint_gammas=last.get("eval_endpoint_gammas", []),
                       eval_goal_loss=last.get("eval_goal_loss", float("nan")),
                       eval_goal_eta=last.get("eval_goal_eta", 0.0),
                       eval_goal_velocity_weight=last.get("eval_goal_velocity_weight", 0.2),
                       eval_endpoint_used=last.get("eval_endpoint_used", 0.0),
                       eval_mode_noise_coupling=last.get("eval_mode_noise_coupling", False),
                       eval_mode_label_known=last.get("eval_mode_label_known", 0.0),
                       negative_loss=last.get("negative_loss", float("nan")),
                       batch_neg=int(last.get("batch_neg", 0)),
                       batch_success=int(last.get("batch_success", 0)),
                       success_replay_size=int(last.get("success_replay_size", 0)),
                       success_replay_added=int(last.get("success_replay_added", 0)),
                       success_replay_composition=(success_replay.composition()
                                                   if success_replay is not None else {}),
...
def main():
    ...
    ap.add_argument("--min-modes-per-gamma", type=int, default=0,
                    help="trajectory modes are diagnostic in window-native gathering; default 0")
    ...
    ap.add_argument("--accepted-window-budget-file", default="",
                    help="JSON iteration->window-count from the Full arm; valid only with "
                         "--ablate-curriculum and used to match its positive sample mass")
    ...
    if args.accepted_window_budget_file:
        if not args.ablate_curriculum:
            raise ValueError("--accepted-window-budget-file is only valid with --ablate-curriculum")
        with open(args.accepted_window_budget_file) as f:
            raw_budget = json.load(f)
        cfg.accepted_window_budget = {int(k): int(v) for k, v in raw_budget.items()}
