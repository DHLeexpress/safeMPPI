...
_RUN = os.path.dirname(_WORK)
_OVERNIGHT = os.path.join(_WORK, "codex_overnight")
sys.path[:0] = [_HERE, _REV, _OVERNIGHT, _RUN, _WORK]
...
if os.path.dirname(os.path.abspath(HP.__file__)) != _REV:
    raise ImportError(f"expected endpoint-free local grid_hp_expt.py, imported {HP.__file__}")
...
def cbf_reward(pos, vel, obs_xy, r_col):
    ...
    # ``r_col`` may be a scalar (legacy equal-radius scene) or one radius per
    # obstacle.  Per-obstacle radii are required by the giant-obstacle OOD
    # benchmark; collapsing them to their mean silently shrinks the giant.
    r_col = torch.as_tensor(r_col, dtype=pos.dtype, device=pos.device)
...
def kazuki_deploy(policy, env, safe_coefs, gamma_ctx=0.5, T=250, reach=0.1, device="cpu", seed=0,
                  rec=None, source_temp=1.0):
    ...
    r_col = torch.tensor(obs[:, 2] + rr + R_MARGIN, dtype=torch.float32, device=device)
    ...
    for t in range(T):
        ...
        if prev_z is None:
            z = source_temp * torch.randn(N_SAMPLE, d, device=device); taus = ODE_TIMES_FULL
        else:
            z = TAU_WARM * prev_z[None].expand(N_SAMPLE, d) \
                + (1.0 - TAU_WARM) * source_temp * torch.randn(N_SAMPLE, d, device=device)
    ...
    return dict(path=np.array(path), controls=np.asarray(hist, dtype=np.float32),
                reached=reached, collided=collided, steps=len(path) - 1)
...
def main():
    ...
    ap.add_argument("--source-temp", type=float, default=1.0,
                    help="source-noise temperature; 1.0 is Mizuta-faithful, lower values match a selected FM controller")
    ...
    if args.viz_out:                                                          # record ONE episode + dump
        rec = []
        out = kazuki_deploy(pol, env, coefs, gamma_ctx=args.gamma_ctx, T=args.T, reach=args.reach,
                            device=dev, seed=args.seed0, rec=rec, source_temp=args.source_temp)
        torch.save(dict(rec=rec, path=out["path"], reached=out["reached"], collided=out["collided"],
                        ...
                        source_temp=args.source_temp), args.viz_out)
    ...
    for m in range(args.M):
        out = kazuki_deploy(pol, env, coefs, gamma_ctx=args.gamma_ctx, T=args.T, reach=args.reach,
                            device=dev, seed=args.seed0 + m, source_temp=args.source_temp)
    ...
    config = dict(w_safe=coefs, gamma_ctx=args.gamma_ctx, coll_w=COLL_W, goal_w=GOAL_W,
                  ...
                  n_elite=N_ELITE, n_copy=N_COPY, source_temp=args.source_temp,
                  obstacle_radius_model="per_obstacle")
