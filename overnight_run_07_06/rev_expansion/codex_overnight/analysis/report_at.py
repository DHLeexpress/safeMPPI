...
def eval_one(ckpt, g, M, outdir, wall_plugs, start_eps, reach, gpu, goal_xy=None, T=250):
    ...
    cmd = [sys.executable, str(P2 / "eval_ae.py"), "policy-worker",
           ...
           "--wall-plugs", str(wall_plugs), "--start-eps", str(start_eps), "--T", str(T)]
    if goal_xy is not None:
        cmd += ["--goal-xy", str(goal_xy[0]), str(goal_xy[1])]
    try:
...
def main():
    ...
    ap.add_argument("--T", type=int, default=250, help="deploy step budget + success cap (raise for slow low-γ)")
    ap.add_argument("--expert-dir", default=str(P2 / "results/expert_gt"))
    ap.add_argument("--outdir", default="")
    args = ap.parse_args()

    outdir = Path(args.outdir) if args.outdir else (P2 / "results/p2" / f"eval_{args.tag}")
    ...
    with ThreadPoolExecutor(max_workers=args.concurrency) as ex:
        futs = [ex.submit(eval_one, args.ckpt, g, args.M, outdir, args.wall_plugs,
                          args.start_eps, args.reach, args.gpu, args.goal_xy, args.T) for g in args.gammas]
    ...
    for g in args.gammas:
        ...
        a = SR == 1.0
        b = CR == 0.0
        cc = np.isfinite(clr) and np.isfinite(eclr) and clr > eclr
        dd = np.isfinite(tim) and np.isfinite(etim) and tim < etim
        ee = cov > ecov
