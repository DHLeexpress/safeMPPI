#!/usr/bin/env bash
# One-arm AFE deep-ensemble launcher. All task and compute choices are explicit.
set -euo pipefail
...
"$PYTHON_BIN" analysis/afe_ensemble_report.py \
  --run "$RUN" \
  --out "$OUT/afe_ensemble_${PROFILE}_report.png"
