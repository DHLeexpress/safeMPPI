...
while true; do
  ...
  if [ "$last2" -ge 2 ]; then
    ...
    NEW="results/p2/unit146_gen${gen}_s$((850 + gen))"
    CUDA_VISIBLE_DEVICES=$GPU setsid nohup python grid_expand_hardtail.py \
      --ckpt "$CKPT" --outdir "$NEW" \
      --iters 82 --drop-train-state --legacy-prime-iters 1 --freeze --lr 2e-5 --seed $((850 + gen)) \
      ...
      --escape-replay "$ESCAPE" --escape-quota 64 --escape-eta 1.0 \
