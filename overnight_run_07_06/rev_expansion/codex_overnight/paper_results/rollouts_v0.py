# _v0 | model: it146 = results/p2/unit_ratchet_gen2_s802/ckpt_146.pt | recipe: corrected guarded unit (open scene; lr 2e-5 frozen enc, recovery .3, hard 12, escape 64, ratcheted teacher, trust gates) | data: ours+expert paths from results/p2/eval_it146_m100 and results/expert_gt (M100 stored per-seed paths, gamma {0.1,0.5,1.0}); Kazuki SUCCESS = results/kazuki_final_m200 (tuned w_safe=.3, coll_w=20, goal_w=2.0, goal_coef=.5); Kazuki TRAPPED = results/kazuki_wsweep (published-style weights coll_w=100 goal_w=.1 goal_coef=.1, single w_safe — every episode times out)
...
def main():
    ...
    fig.tight_layout()
    for ext in ("png", "pdf"):
        fig.savefig(os.path.join(HERE, f"rollouts_v0.{ext}"), dpi=135, bbox_inches="tight")
    print("wrote rollouts_v0.png/.pdf")
