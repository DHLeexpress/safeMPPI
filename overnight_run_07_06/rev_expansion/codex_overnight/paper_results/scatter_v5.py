...
SERIES = [
    ...
    ("Our approach", "results/p2/eval_faithful_div_it100", "*", 240, dict(edgecolors="k", linewidths=0.9)),
    ("Pretrained", "results/p2/eval_pretrained_g47", "s", 70, dict(alpha=0.55)),
]
# ablation brothers (add if their eval dirs exist)
BROTHERS = [(r"$-$SOCP", "results/p2/eval_faithbro_div_nosocp", "P", 80, dict(alpha=.8)),
            (r"$-$Progress", "results/p2/eval_faithbro_div_noprog", "X", 80, dict(alpha=.8)),
            (r"$-$Curriculum", "results/p2/eval_faithbro_div_nocur", "D", 60, dict(alpha=.8))]
...
def main():
    ...
    for ext in ("png", "pdf"):
        fig.savefig(os.path.join(HERE, f"scatter_v5.{ext}"), dpi=140, bbox_inches="tight")
    print("wrote scatter_v5.png/.pdf")
