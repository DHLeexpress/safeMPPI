# _v1 | model: it146 = results/p2/unit_ratchet_gen2_s802/ckpt_146.pt | recipe: corrected guarded unit (lr 2e-5, frozen enc, recovery .3, hard-quota 12, escape 64, ratcheted teacher, trust gates, OPEN scene) | data: UNCHANGED from _v0 (model still it146): expert=results/expert_gt M100 full gamma sweep, ours=results/p2/eval_it146_m100 M100, kazuki=results/kazuki_final_m200 M200 (tuned single w_safe=.3 coll_w=20 goal_w=2.0 goal_coef=.5 beta_mppi=20 on OUR pretrained model, gamma-conditioned per point), pretrained=results/p2/eval_pretrained_m25 M25 (context) | _v1 bump: lockstep with table_v1 (same-scene ablation update; ablations do not appear in this figure)
...
def main():
    ...
    fig.tight_layout()
    for ext in ("png", "pdf"):
        fig.savefig(os.path.join(HERE, f"scatter_v1.{ext}"), dpi=140, bbox_inches="tight")
    print("wrote scatter_v1.png/.pdf")
