...
sys.path.insert(0, str(HERE))  # local algorithm shadows (e.g. boundary-adapter checkpoint loader)
...
def _apply_goal(env, args):
    """Move the goal (e.g. cleared (4.7,4.7)); propagates to the reach/success check, relgoal, planner."""
    gxy = getattr(args, "goal_xy", None)
    if gxy is not None:
        import torch as _t
        env.goal = _t.tensor([float(gxy[0]), float(gxy[1])], dtype=env.goal.dtype)
    return env
...
def summarize_paths(paths, env, gamma: float, method: str, reach: float = 0.1,
    ...
    for raw in paths:
        ...
        sid = GM2.staircase_id_goal(p, goal, reach)   # goal-relative (no-op at (5,5); fixes moved goal)
        if sid is not None:
            ids.add(sid)
...
def expert_worker(args) -> None:
    env = GS.make_grid()
    _apply_wall_plugs_eval(env, getattr(args, 'wall_plugs', 0)); _apply_goal(env, args)
    env.T = int(getattr(args, 'T', env.T) or env.T)   # success cap = step budget (raise for slow low-γ)
    cfg = GS.mode1_config()
...
def policy_worker(args) -> None:
    ...
    env = GS.make_grid()
    _apply_wall_plugs_eval(env, getattr(args, 'wall_plugs', 0)); _apply_goal(env, args)
    env.T = int(getattr(args, 'T', env.T) or env.T)   # success cap = deploy budget (raise for slow low-γ)
...
def build_parser():
    ...
    common.add_argument("--goal-xy", type=float, nargs=2, default=None,
                        help="move the goal, e.g. 4.7 4.7 (cleared). Default (5,5).")
    common.add_argument("--T", type=int, default=250,
                        help="step budget = deploy horizon AND success cap (env.T); raise for slow low-γ")
