...
sys.path.insert(0, _PARENT); sys.path.insert(0, _HERE)
...
def kazuki_deploy(policy, env, safe_coefs, gamma_ctx=0.5, T=250, reach=0.1, device="cpu", seed=0, rec=None):
    ...
    r_col = float(obs[:, 2].mean()) + rr + 0.05
    ...
    for t in range(T):
        ...
        if rec is not None:
            ...
            rec.append(dict(state=st.copy(), **vz))
        else:
...
def main():
    ...
    if args.viz_out:                                                          # record ONE episode + dump
        rec = []
        out = kazuki_deploy(pol, env, coefs, gamma_ctx=args.gamma_ctx, T=args.T, device=dev, seed=args.seed0, rec=rec)
        torch.save(dict(rec=rec, path=out["path"], reached=out["reached"], collided=out["collided"],
                        gamma_ctx=args.gamma_ctx, w_safe=coefs), args.viz_out)
    ...
    for m in range(args.M):
        out = kazuki_deploy(pol, env, coefs, gamma_ctx=args.gamma_ctx, T=args.T, device=dev,
                            seed=args.seed0 + m)
        n_reach += int(out["reached"]); n_coll += int(out["collided"]); steps.append(out["steps"])
        if len(paths) < 10:
            paths.append(out["path"].tolist())
    ...
    res = dict(tag=args.tag, w_safe=coefs, gamma_ctx=args.gamma_ctx, M=args.M,
               SR=n_reach / args.M, CR=n_coll / args.M, mean_steps=float(np.mean(steps)),
               ckpt=os.path.basename(args.ckpt), paths=paths)
