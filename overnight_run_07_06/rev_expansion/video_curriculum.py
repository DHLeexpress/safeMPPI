"""ULTIMATE curriculum video (user 2026-07-09). One MP4 frame per selected iteration of an expansion run:
...
Usage: python video_curriculum.py --run results/video_src/m_combo --out video/m_combo_curriculum.mp4
       [--ckpt ../results/hp_repr/pretrained_a32uni.pt] [--sig-abs 0.25] [--pfloor 0.3] [--qmarg 0.33]
"""
...
mpl.rcParams.update({'font.family': 'serif', 'mathtext.fontset': 'cm',
                     'axes.titlesize': 17, 'axes.labelsize': 15, 'xtick.labelsize': 13,
...
def scene(ax):
    ...
    ax.plot(0, 0, 's', c='k', ms=7, zorder=8); ax.plot(5, 5, '*', c='gold', mec='k', ms=15, zorder=8)
...
def frame(fig, it, db, rec, recs_upto, pre_path, sig_lims, args):
    ...
    if it == 0:
        ...
        em = db['lab'] == 'easy'
        axA.scatter(db['pos'][~em, 0], db['pos'][~em, 1], c=db['sig'][~em], cmap='plasma',
                    vmin=sig_lims[0], vmax=sig_lims[1], s=22, zorder=4, linewidths=0)
        if em.any():
            axA.scatter(db['pos'][em, 0], db['pos'][em, 1], c=db['sig'][em], cmap='plasma',
                        vmin=sig_lims[0], vmax=sig_lims[1], s=85, zorder=5,
                        edgecolors=C_E, linewidths=2.4)
        axA.set_title(f'(A) {len(db["lab"])} windows: {int(em.sum())} easy (ring) / '
                      f'{int((~em).sum())} frontier\n{len(db["paths"])} gathered rollouts (dashed)',
                      fontsize=15)
    sm = mpl.cm.ScalarMappable(cmap='plasma', norm=mpl.colors.Normalize(*sig_lims))
    ...
    if db is not None and it > 0:
        ...
        em = db['lab'] == 'easy'
        if (~em).any():
            axB.hist(db['sig'][~em], bins=bins, color=C_F, alpha=.65, label=f'frontier {int((~em).sum())}')
        if em.any():
            axB.hist(db['sig'][em], bins=bins, color=C_E, alpha=.75, label=f'easy {int(em.sum())}')
        if args.sig_abs > 0:
            axB.axvline(args.sig_abs, color='k', ls=':', lw=2)
            axB.text(args.sig_abs, axB.get_ylim()[1] * .95, r' $\sigma_{abs}$', fontsize=13, va='top')
        axB.legend()
    else:
    ...
    if db is not None and it > 0:
        em = db['lab'] == 'easy'
        for m, c, s_ in ((~em, C_F, 14), (em, C_E, 42)):
            if m.any():
                axC.scatter(db['marg'][m], db['prog'][m], db['sig'][m], c=c, s=s_, alpha=.7, depthshade=True)
        ml = (min(-0.05, float(db['marg'].min()) - 0.02), max(.5, db['marg'].max()))
        pl = (0, max(.8, db['prog'].max())); zl = sig_lims
        m_thr = float(np.quantile(db['marg'], args.qmarg))
        ...
        axC.plot_surface(xx, np.full_like(xx, args.pfloor), zz2, color='#ff7f0e', alpha=.15, shade=False)
        if args.sig_abs > 0:
            xx3, yy3 = np.meshgrid(np.linspace(*ml, 2), np.linspace(*pl, 2))
            axC.plot_surface(xx3, yy3, np.full_like(xx3, args.sig_abs), color='#9467bd', alpha=.18, shade=False)
    ...
    axC.set_xlabel('SOCP margin', fontsize=13); axC.set_ylabel('net-progress', fontsize=13)
    axC.set_zlabel(r'$\sigma$', fontsize=15)
    axC.set_title(f'(C) $\\sigma$ on Z — validity RED: m>0, prog$\\geq${args.vpf}\n'
                  f'frontier: m$\\leq$q{args.qmarg} | prog$\\geq${args.pfloor}'
                  + (r' | $\sigma_{abs}$' if args.sig_abs > 0 else ''), fontsize=13)
    axC.view_init(elev=18, azim=-55); axC.tick_params(labelsize=10)
    # ---------- D: 5 bins + arrows ----------
...
def main():
    ...
    ap.add_argument('--title', default='m_combo curriculum')
    ap.add_argument('--sig-abs', type=float, default=0.25)
    ap.add_argument('--pfloor', type=float, default=0.3)
    ap.add_argument('--qmarg', type=float, default=0.33)
    ...
    FR = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'video', '_frames')
