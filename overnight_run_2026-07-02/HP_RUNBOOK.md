...
| run name | override flags | tests |
|---|---|---|
| beta0.05 | `--beta 0.05` | tilt greediness (β0.01 & β0.2 done locally) |
| beta0.3 | `--beta 0.3` | softer tilt |
| enc0.1 | `--enc-lr-mult 0.1` | partial encoder slowing (0.0 done locally) |
| lr5e-5 | `--lr 5e-5` | mid step-size (1e-5 done locally) |
| **dfrac0.1** | `--demo-frac 0.1` | 2.1 distillation: 10% of every batch = demo windows |
| **dfrac0.25** | `--demo-frac 0.25` | 2.1 default (32 demo + 96 positives per 128-batch) |
| **dfrac0.5** | `--demo-frac 0.5` | 2.1 heavy replay (the old v1 anchor regime) |
| **lwf0.01** | `--lwf-eta 0.01` | 2.2 LwF: light field-distillation on demo ctx |
| **lwf0.1** | `--lwf-eta 0.1` | 2.2 default |
| **lwf1.0** | `--lwf-eta 1.0` | 2.2 strong hold |
| dfrac0.25+lwf0.1 | `--demo-frac 0.25 --lwf-eta 0.1` | the combined hold (run after singles) |
...
Reference results from the local machine (base, 1000-iter arms): plain expansion DEGRADES validity at every
single-knob setting tried — beta0.01: 68→36% · enc0: 79→35% (drift≡0 → forgetting is in the FIELD, not only the
encoder) · lr1e-5: pending. **That is why 2.1/2.2 exist**: hold the field on old inputs (replay / LwF) while the
quota-free buffer explores. Expected winner: dfrac or lwf arms holding val2 while cov_cum climbs ≥ the ~13%/1k-iter
discovery rate.
