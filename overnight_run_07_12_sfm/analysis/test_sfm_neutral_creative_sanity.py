...
def test_trigger_requires_authenticated_failed_autonomous_delivery(tmp_path):
    gamma_path = _write(tmp_path / "gamma.json", {
        "status": GAMMA.STATUS,
        "ci_clean_four_metric_win": False,
        "source_commit": C.EXPECTED_TRIGGER_SOURCE,
    })
    delivery = {
        "status": AUTO.STATUS,
        ...
        "r100_training_delivery": "/run/r100.json",
    ...
    result = C._validate_trigger(tmp_path / "trigger.json", trigger)
    assert result["delivery"]["selected_arm"] == "lr1em5_s04"

    trigger["autonomous_delivery_sha256"] = "0" * 64
    with pytest.raises(RuntimeError, match="digest mismatch"):
        C._validate_trigger(tmp_path / "trigger.json", trigger)
...
def test_encoder_stage_stops_on_drift_regression_or_cr(tmp_path):
    ...
    gate = C._encoder_stage_gate(delivery, candidate, control)
    assert gate["unsafe"]
