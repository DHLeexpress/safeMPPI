"""ep16 deep study (single-integrator) with the polytope bimodal mixture proposal — FULL-scene camera (not ego).
ONE episode (ep16) x gamma {0.1,0.5,1.0}, 2 rows (accept/reject trajectories on the polytope + bimodal control
mean/cov). num_samples=256.
  python overnight_run_2026-06-28/ep16_study.py
"""
...
CFG = dict(horizon=10, dt=DT, num_samples=256, noise_sigma=(0.5, 0.5), u_min=(-2., -2.), u_max=(2., 2.),
           ...
           use_polytope_barrier=True, use_goal_nominal=False, warm_start=True, centroid_gain=0.1, centroid_smooth=0.5,
           sigma_volume_gain=0.5, sigma_aniso=2.0, predict_gain=0.4, polytope_nbase=16)


def rollout(g, steps=80, dev="cpu"):
    s0, goal, obs, vel = _load("ucy", EP, steps); ad = SafeMPPIAdapter(**CFG)
    st = s0.astype(np.float32).copy(); rec = []; reached = False; path = [st[:2].copy()]
    ...
    for t in range(steps):
        ...
        if not reached:
            ...
            st = _si_step(st, a.detach().cpu().numpy(), DT)
...
def main():
    data = {g: rollout(g) for g in GAMMAS}; print("rolled ep16", flush=True)
    s0, goal, obs, _ = _load("ucy", EP, 80)
    a = obs[..., :2].reshape(-1, 2); a = a[~np.isnan(a).any(1)]
    pts = np.vstack([a, s0[:2][None], goal[None]]); pad = 1.0
    xl = (pts[:, 0].min() - pad, pts[:, 0].max() + pad); yl = (pts[:, 1].min() - pad, pts[:, 1].max() + pad)
    fig, axes = plt.subplots(2, len(GAMMAS), figsize=(4.0 * len(GAMMAS), 7.6), squeeze=False)
    nF = max(len(data[GAMMAS[0]][0]), 1)

    def draw(f):
        for ci, g in enumerate(GAMMAS):
            rec, path, goal = data[g]; st = rec[min(f, len(rec) - 1)]; p = st["p"]
            ...
            if st["cpos"] is not None:
                cp = st["cpos"]; ax.annotate("", xy=(cp[0], cp[1]), xytext=(p[0], p[1]), arrowprops=dict(arrowstyle="-|>", color="#ff7f00", lw=1.8), zorder=9)
                ax.scatter([cp[0]], [cp[1]], s=18, c="#ff7f00", zorder=9)
            ...
            ax.set_title(f"γ={g}", fontsize=11); ax.set_xlim(*xl); ax.set_ylim(*yl); ax.set_aspect("equal"); ax.set_xticks([]); ax.set_yticks([])
            if ci == 0: ax.set_ylabel("ep16  trajectories (full scene)", fontsize=8)
            axc = axes[1][ci]; axc.clear(); fc = st["fc"]; fcf = st["fcf"]; sm = st["smean"]; sg = st["sigma"]
            ...
            if ci == 0: axc.set_ylabel("ep16  bimodal μ/Σ", fontsize=8)
        fig.suptitle(f"ep16 study (SI) · green=accepted/red=rejected · orange=robot→EXACT centroid + bimodal μ/Σ · "
                     f"navy ✗=executed · full scene · t={f}", fontsize=10)
        return []

    anim = FuncAnimation(fig, draw, frames=nF, interval=200)
    out = os.path.join(FIG, "ep16_study.gif"); anim.save(out, writer=PillowWriter(fps=6), dpi=80)
    print("saved", out); draw(35); fig.savefig(os.path.join(FIG, "ep16_study.png"), dpi=110); print("saved png")
