...
# Zeroth fix = proximity-adaptive speed cap (umax_react_dist); carried by all later rows.
ADP = dict(umax_react_dist=2.0)
ROWS = [
    ("baseline\nu_max=2 fixed",         dict(),                                                                       0.0),
    ("0: u_max->adaptive react=2.0",    dict(ADP),                                                                    0.0),
...
def main():
    ...
    def draw(f):
        ...
        fig.suptitle(f"ep15 diagnostic · accept=green / reject=red ✗ trajectories · red=executed path · t={f}", fontsize=11)
        return []

    anim = FuncAnimation(fig, draw, frames=nF, interval=1000 // args.fps)
    p = os.path.join(FIG, "ep15_diagnostic.gif")
    anim.save(p, writer=PillowWriter(fps=args.fps), dpi=80); print("saved", p)
    draw(40); fig.savefig(os.path.join(FIG, "ep15_diagnostic.png"), dpi=110); print("saved png")
    # verdict table
